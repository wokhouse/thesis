...
inclusive_anova <- mod_data %>%
  aov(inclusive_behavior ~ cis + trans + nonbinary + man + woman + man:trans + woman:trans + nonbinary:man + nonbinary:woman + ethnicity + year + major + age + sexuality, .) %>%
  tidy() %>%
  mutate_if(is.numeric, round, digits = 2)
# congruence tukey
...
mod_data %>%
  mutate(gender_bin = factor(case_when(
    ...
    cis == FALSE & nonbinary == TRUE & trans == FALSE & man == FALSE & woman == FALSE ~ "Non-binary",
    cis == FALSE & nonbinary == TRUE & trans == TRUE & man == FALSE & woman == FALSE ~ "Trans Non-binary",
    TRUE ~ "other"
  ), levels = c("Cis Man","Cis Woman","Trans Man","Trans Woman","Non-binary","Trans Non-binary"))) %>%
  filter(gender_bin != "other", !is.na(inclusive_behavior)) %>%
