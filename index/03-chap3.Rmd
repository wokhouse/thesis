...
```{r comfort_sharing, echo = FALSE}
comfort_data <- mod_data %>%
  select(
    gender_bin,
    pronoun_bin,
    starts_with("comfort_")
  ) %>%
  filter(complete.cases(.)) %>%
  pivot_longer(starts_with("comfort_"), names_to = "factor", values_to = "val")
comfort_anova_raw <- comfort_data %>%
  aov(val ~ factor + pronoun_bin + gender_bin + factor:gender_bin + factor:pronoun_bin, .)
comfort_anova <- comfort_anova_raw %>% tidy()
comfort_tukey <- comfort_anova_raw %>%
  TukeyHSD() %>%
  tidy() %>%
  filter(adj.p.value < 0.05)
comfort_desc_factor <- comfort_data %>%
  group_by(factor) %>%
  summarise(mean = mean(val), sd = sd(val))
comfort_desc_pronoun <- comfort_data %>%
  group_by(pronoun_bin) %>%
  summarise(mean = mean(val), sd = sd(val))
comfort_desc_gender <- comfort_data %>%
  group_by(gender_bin) %>%
  summarise(mean = mean(val), sd = sd(val))
comfort_desc_factorgender <- comfort_data %>%
  group_by(factor, gender_bin) %>%
  summarise(mean = mean(val), sd = sd(val))
comfort_desc_factorpronoun <- comfort_data %>%
  group_by(factor, pronoun_bin) %>%
  summarise(mean = mean(val), sd = sd(val))
```
Items 1, 2, 3, 7, 8, and 9 touched on participant's comfort sharing their pronouns.
A factorial ANOVA found x effect of setting and x effect of a third-party's presence or initiation on sharing pronouns. 
...
```
Visualization of the first and second component indicated possible bimodality along the first component. Hartigan's dip test confirmed this, `r comp1_bimodal$t[1]`. This indicates that experiences with cisnormativity may be described as bimodal, likely indicating that cisgender and non-cisgender people have significantly different experiences navigating the world as their gender. 
...
comp3_desc <- pca_comp3_out %>%
  ...
  mutate(t = paste("(*M* = ", mean, ", *SD* = ", sd, ")", sep = ""))
```
**Am I interpreting this right?** A final one-way ANOVA demonstrated a significant effect of gender on the third component, `r comp3_anova$t[1]`. Non-binary people, `r comp1_desc$t[6]`, are significantly less concerned that sharing their pronouns will draw unwanted attention than cisgender men, `r comp1_desc$t[1]`, cisgender women, `r comp1_desc$t[2]`, transgender men `r comp1_desc$t[3]`, and transgender women `r comp1_desc$t[4]`. There were no significant effects between any other groups.

```{r comp3_viz, echo = FALSE, fig.widht = 10, fig.height= 10}
