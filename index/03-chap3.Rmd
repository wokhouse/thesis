...
pca_in <- mod_data %>%
  ...
  mutate(gender_bin = as.numeric(gender_bin), nonbinary = as.numeric(nonbinary), trans = as.numeric(trans), cis = as.numeric(cis)) %>%
  mutate(pronoun_bin = recode(pronoun_bin, "she/her" = 1, "he/him" = 2, "they/them" = 3, "she/her & they/them" = 5, "he/him & they/them" = 6, "all pronouns" = 7, "he/him & she/her" = 7)) %>%
...
variance <- pca_out %>%
  tidy("pcs")
eigenvals <- get_eigenvalue(pca_out) %>% mutate_if(is.numeric, round, digits = 1)
```
...
loadings %>%
  ...
  kable(
    "html",
    escape = FALSE,
    col.names = c("Item", "Comp. 1 Loading", "Comp. 2 Loading", "Comp. 3 Loading")
  )
```
Visualization of the first two components (account for 47.6% of the variation) yielded interesting results. The x-axis represents the first component. Loadings indicate that this component represents one's feelings of consistency between gender, appearance and pronouns. Negative values along the x-axis likely indicate that one is misgendered less frequently and other people usually correctly assume one's pronouns. Positive values along the x-axis likely indicate that one is misgendered more frequently and other people usually do not correctly assume one's pronouns. 

The y-axis represents the second component. Loadings indicate that this component represents one's desire to share one's pronouns and, to a smaller extent, concern that doing so will draw attention to oneself. Positive values along the y-axes indicate that one has *blah blah blah i need to do more exploratory analysis before this exploratory analysis*
```{r pca_vis, echo = FALSE}
pca_out %>%
  fviz_pca_ind(geom.ind = "point", pointshape = 21, 
               pointsize = 2, 
               fill = factor(
                 pca_in$gender_bin,
                 labels = c("Cis Man","Cis Woman","Trans Man","Trans Woman", "Cis Non-binary", "Non-binary","Trans Non-binary")
               ),
               palette="jol",
               col.ind = "black", 
               addEllipses = TRUE,
               ...
               legend.title = "Gender")
```
Cisgender men and cisgender women appear very separate from the non-cisgender participants. In this plot, cis men are surrounded surrounded by a red circle to the left of the y-axis. Cis women are surrounded by an orange circle to the left of the y-axis. There is significant overlap between cis men and cis women. 

Notably, there is no overlap between cisgender men and transgender women (surrounded by a blue-green circle) along the x-axis, indicating that cisgender men and transgender women have very different experiences with the relationship between their gender and appearance, as well as how frequently they are misgendered. 
